...
- ✋ **마커 드래그 이동** — 편집 모드에서 마커를 길게 눌러 새 위치로 이동
...
3. **편집 모드 켜기** — 좌측 상단의 자물쇠 아이콘(🔒 보기 → ✏️ 편집 중)을 눌러 편집 모드 진입
4. 사이드바 또는 모바일 헤더의 레이어 아이콘에서 사용할 종류 선택
5. **편집 모드에서**: 빈 곳을 짧게 클릭/탭 → 빤짝이는 마커 생성
6. **마커 이동** (편집 모드): 배치된 마커를 잠깐 누른 채로 드래그 → 원하는 위치로 옮기기
7. **마커 편집** (편집 모드): 마커를 짧게 탭 → 종류/메모 변경 또는 삭제
8. **편집 종료** — 다시 토글 버튼을 눌러 보기 모드로 (실수로 마커가 추가/이동되는 걸 방지)
9. **여러 지도 전환** — 상단 탭이나 사이드바 목록에서 지도 선택
10. 설정 아이콘(⚙️)으로 마커 종류 언제든 추가/편집

### 보기 모드 vs 편집 모드

- **보기 모드 (기본)** 🔒 — 지도와 마커를 자유롭게 보고 확대/축소/이동만 가능. 실수로 마커가 추가되거나 이동되지 않음.
- **편집 모드** ✏️ — 빈 공간 클릭으로 마커 추가, 마커 클릭으로 편집, 마커 드래그로 이동 가능. 화면 상단에 노란 배지가 뜨고 테두리가 천천히 빛남.
- 새로고침해도 모드가 유지되지만, **브라우저/탭을 닫으면 자동으로 보기 모드로 초기화**되어 안전합니다.
