...
- ✋ **마커 드래그 이동** — 배치된 인원/표시를 길게 눌러 새 위치로 이동
...
4. 지도의 원하는 지점을 짧게 클릭/탭 → 빤짝이는 마커 생성
5. **마커 이동**: 배치된 마커를 잠깐 누른 채로 드래그 → 원하는 위치로 옮기기 (모바일에서는 길게 눌러서 들어올린 뒤 이동)
6. **마커 편집**: 마커를 짧게 탭 → 종류/메모 변경 또는 삭제
7. **여러 지도 전환** — 상단 탭이나 사이드바 목록에서 지도 선택
8. 설정 아이콘(⚙️)으로 마커 종류 언제든 추가/편집
